...
const ChatLabel = ({ openMenu, setOpenMenu, id, name  }) => {
  ...
  const deleteHandler = async () =>{
    try {
      const confirm = window.confirm('Are you sure you want to delet this chat?')
      if(!confirm) return
  ...
  return (
    <div  onClick={selectChat} className='flex items-center justify-between p-2 text-white/80 hover:bg-white/10 rounded-md cursor-pointer text-sm group'>
      <p className='group-hover:max-w-[85%] truncate'>{name}</p>
